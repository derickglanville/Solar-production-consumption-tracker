(function () {
  ...
  const LATITUDE = 41.2706;
  const LONGITUDE = -73.7774;
  ...
  function solarPosition(date) {
    ...
    const sidereal = normalize(280.46061837 + 360.98564736629 * days + LONGITUDE);
    ...
    const azimuth = normalize(degrees(Math.atan2(
      ...
      Math.cos(radians(hourAngle)) * Math.sin(radians(LATITUDE)) - Math.tan(radians(declination)) * Math.cos(radians(LATITUDE))
    )) + 180);
    return { azimuth, elevation };
  }

  function solarEvents(date) {
    const parts = yorktownParts(date);
    ...
    const hourAngle = degrees(Math.acos(clamp(
      Math.cos(radians(90.833)) / (Math.cos(radians(LATITUDE)) * Math.cos(declination)) - Math.tan(radians(LATITUDE)) * Math.tan(declination),
      -1, 1
    )));
    const timezoneEastMinutes = -date.getTimezoneOffset();
    const noon = 720 - 4 * LONGITUDE - equationTime + timezoneEastMinutes;
  ...
  function renderArc(root, now, events) {
    const points = [];
    let peakElevation = 0;
    for (let minute = events.sunrise; minute <= events.sunset; minute += 15) {
      const position = solarPosition(dateAtLocalMinute(now, minute));
    ...
    const currentPosition = solarPosition(now);
  ...
  function renderHourMarkers(root, now, events, geometry) {
  ...
  function renderHourlyRows(root, now, events, geometry, profile, entry) {
    ...
    renderHourMarkers(root, now, events, geometry);
  }
  ...
  function render(root) {
    ...
    const systemSize = Number(root.dataset.systemSize || 18.45);
    const power = clamp(systemSize * irradiance / 1000 * 0.86, 0, systemSize * 0.96);
    const localTime = new Intl.DateTimeFormat("en-US", { timeZone: TIME_ZONE, weekday: "short", month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }).format(reference);
    ...
    const sunrisePosition = solarPosition(dateAtLocalMinute(reference, events.sunrise));
    const sunsetPosition = solarPosition(dateAtLocalMinute(reference, events.sunset));
    const daylightHours = Math.max(0, events.sunset - events.sunrise) / 60;
    const dateLabel = new Intl.DateTimeFormat("en-US", { timeZone: TIME_ZONE, month: "long", day: "numeric", year: "numeric" }).format(reference);
    ...
    if (sky) sky.dataset.condition = skyClass(weather, position.elevation);
    ...
    setText(root, "[data-solar-editorial-meta]", `Yorktown Heights, NY · 41.2706° N, 73.7774° W · Eastern Time`);
    ...
    setText(root, "[data-solar-sky-title]", `${displayWeather} · ${Math.round(cloud)}% cloud cover`);
    setText(root, "[data-solar-sky-detail]", position.elevation > 0 ? `${Math.round(irradiance)} W/m² is supporting an estimated ${power.toFixed(1)} kW now.` : "The sun is below the horizon, so modeled solar generation is 0 kW.");
    ...
    renderHourlyRows(root, reference, events, geometry, profile, entry);
  }
