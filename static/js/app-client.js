...
function updateEntriesMeterDifferenceSummary(entries, meterDifferences, allEntries = entries) {
  ...
  const monthKeys = Array.from({ length: 3 }, (_, offset) => {
    const month = new Date(`${getTodayIsoDate().slice(0, 7)}-01T12:00:00`);
    month.setMonth(month.getMonth() - offset);
    return `${month.getFullYear()}-${String(month.getMonth() + 1).padStart(2, "0")}`;
  });
  const monthLabels = monthKeys.map((monthKey) => new Date(`${monthKey}-01T12:00:00`).toLocaleDateString("en-US", {
    month: "short",
    year: "numeric"
  }));
  ...
  const monthlyTotals = monthKeys.reduce((result, monthKey) => {
    result[monthKey] = {
      power: 0, m01: 0, m02: 0, edc: 0, dayBalance: 0,
      powerCount: 0, m01Count: 0, m02Count: 0, edcCount: 0, dayBalanceCount: 0
    };
    return result;
  }, {});
  ...
  const creditBankByMonth = new Map();
  const monthlyCreditChangeByMonth = new Map();
  ...
  const formatCreditBankCard = (bankKwh, label, kwhTarget, valueTarget, { signed = false } = {}) => {
    ...
    const explanation = `Estimated ${label} from tracked meter history. ${direction}${bankKwh.toFixed(1)} kWh × $${electricRate.toFixed(3)}/kWh ≈ ${formatCurrency(estimatedValue)}. This is not the official NYSEG balance.`;
  ...
  formatCreditBankCard(
    ...
    latestBankMonth ? `through ${new Date(`${latestBankMonth}-01T12:00:00`).toLocaleDateString("en-US", { month: "long", year: "numeric" })}` : "",
