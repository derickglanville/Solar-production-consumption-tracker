...
function updateEntriesMeterDifferenceSummary(entries, meterDifferences, allEntries = entries) {
  ...
  ["current", "previous", "earlier"].forEach((position, index) => {
    ...
    target.textContent = `${bankKwh.toFixed(1)} kWh · ${formatCurrency(estimatedValue)}`;
    target.title = `Estimated credit bank at the end of ${monthLabels[index]} from tracked meter history. ${bankKwh.toFixed(1)} kWh × $${electricRate.toFixed(3)}/kWh = ${formatCurrency(estimatedValue)}. This is not the official NYSEG balance.`;
  });
